...
export function isCriticalQuestion(question: string): boolean {
  return /\b(bad fit|poor fit|wrong fit|good fit|suited (for|to)|better suited|downside|red flag|worst fit|wouldn'?t (work|fit)|struggle (with|in)|risks? (of|in)|argue against|why shouldn'?t|why not hire|talk me out of|convince me not to|case against|reasons? not to hire|weakness(es)?|shortcoming|cons? of hiring|limitations?|what does .*lack|lacks?\b.*compared)\b/i.test(question);
}
