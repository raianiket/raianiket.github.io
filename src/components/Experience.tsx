...
const roles = [
  {
    ...
    bullets: [
      ...
      "Mentor engineers through regular PR reviews and technical discussions; conduct technical interviews.",
    ],
  },
  {
    ...
    bullets: [
      ...
      "Owned end-to-end architecture and implementation of Slack eDiscovery Search across UI (React), backend (PHP), and database (PostgreSQL), from Slack data ingestion and event handling to the full frontend search and results interface.",
      "Owned the redesign of the Bulk Export feature, extending the PHP export backend to support multi-user exports under a single URL; previous implementation handled only single user-level export, refactored to process multiple users in one request with unified download link generation.",
      ...
      "Owned the design and build of 4 DAL AI-Agents (Database Health, Restore/Export, StartBackup, PostGraphile Slow-Queries) to automate operational monitoring and resolve issues before they impact customers.",
      ...
      "Designed the parent/child schema and mutation layer for Add-On Actions (eDiscovery, BDI) including Dismiss, Hold/Release, Archive On-Demand, Remove Sharing, and Deletion.",
      "Led development of Gen4 & Gen5 cloud pages (Reports, Restore, Export, Dashboard) using a config-driven template so any new cloud is automatically supported across all pages.",
      "Led code quality initiatives that resolved all critical SonarQube security violations (to 0) and cut total issues by 90%.",
    ],
  },
  {
    ...
    bullets: [
      ...
      "Owned development of a Timeseries and Batch Framework with multi-environment version support scheduling MVW refresh jobs on login and on schedule; applied CPU-level optimizations for improved throughput.",
      ...
      "Owned the integration of ELK Stack with Grafana, establishing centralized backend error log collection and real-time CM error monitoring.",
